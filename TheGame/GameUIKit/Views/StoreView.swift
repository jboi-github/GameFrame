...
private struct ProductRow<C, S>: View where C: GameConfig, S: GameSkin {
...
private struct ProductsView<C, S>: View where C: GameConfig, S: GameSkin {
    let consumableIds: [String]
    let nonConsumableIds: [String]
    let isOverlayed: Bool
    @EnvironmentObject private var skin: S
    ...
    var body: some View {
        ...
        return VStack {
            NavigationArea<C, S>(
                parent: "Store",
                items: [[.Buttons(button: .Restore()), .Links(link: .Back())]],
                ...
                GeometryReader {
                    ...
                    ScrollView {
                        ForEach(0..<products.count, id: \.self) {
                            ...
                            ProductRow<C, S>(product: products[id], isOverlayed: self.isOverlayed, proxy: proxy)
                        }
                    }
                }
            }
